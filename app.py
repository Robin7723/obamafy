...
class SimParams:
    ...
    convergence_force: float = 400.0
    dye_dissipation: float = 1.0
    vel_dissipation: float = 1.0
    pressure_iterations: int = 10
...
class FluidApp(moderngl_window.WindowConfig):
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser):
        parser.add_argument(
            ...
            nargs="+",
            required=True,
            help="One or more target image paths or directories",
        )
        parser.add_argument(
            "--auto_transition",
            type=int,
            default=0,
            help="Interval in milliseconds to automatically switch to the next target image. 0 disables it.",
        )
        parser.add_argument(
            "--seed_image",
            type=str,
            default=None,
            help="Optional starting image for the dye field",
        )
        parser.add_argument(
            "--scale",
            type=float,
            default=1.0,
            help="Scale the sim relative to image size",
        )
        parser.add_argument(
            "--sim_w",
            type=int,
            default=768,
            help="Simulation width (overrides image if provided)",
        )
        parser.add_argument(
            "--sim_h",
            type=int,
            default=0,
            help="Simulation height (0 = preserve aspect from image/sim_w)",
        )
    ...
    def __init__(self, **kwargs):
        ...
        for p in args.images:
            ...
            if path.is_dir():
                dir_files = sorted(
                    [f for f in path.iterdir() if f.suffix.lower() in valid_extensions]
                )
        ...
        self.transition_interval_s = (
            args.auto_transition / 1000.0 if args.auto_transition > 0 else 0
        )
        ...
        tex_args = dict(components=4, dtype="f4")
        ...
        # Upload target image
        self.tex_target.write(
            self.target_images_np[self.current_target_index].tobytes()
        )
        ...
        # Sampler
        self.sampler_linear = self.ctx.sampler(
            filter=(moderngl.LINEAR, moderngl.LINEAR), repeat_x=True, repeat_y=True
        )
        self.sampler_nearest = self.ctx.sampler(
            filter=(moderngl.NEAREST, moderngl.NEAREST), repeat_x=True, repeat_y=True
        )
        self.sampler_mipmap = self.ctx.sampler(
        ...
        self.prog_advect = self.load_program(
            shader_dir / "fullscreen.vert", shader_dir / "advect.frag"
        )
        self.prog_divergence = self.load_program(
            shader_dir / "fullscreen.vert", shader_dir / "divergence.frag"
        )
        self.prog_jacobi = self.load_program(
            shader_dir / "fullscreen.vert", shader_dir / "jacobi.frag"
        )
        self.prog_projection = self.load_program(
            shader_dir / "fullscreen.vert", shader_dir / "projection.frag"
        )
        self.prog_apply_forces = self.load_program(
            shader_dir / "fullscreen.vert", shader_dir / "apply_forces.frag"
        )
        self.prog_render = self.load_program(
            shader_dir / "fullscreen.vert", shader_dir / "render.frag"
        )

        # Uniforms constant across frames
        for prog in (
            self.prog_advect,
            self.prog_divergence,
            self.prog_jacobi,
            self.prog_projection,
            self.prog_apply_forces,
            self.prog_render,
        ):
        ...
        self.initial_delay_seconds = 1.5
        self.simulation_started = False
        print(f"✓ Simulation will start in {self.initial_delay_seconds} seconds.")
    ...
    def _print_controls(self):
        """Prints the keyboard and mouse controls to the console."""
        print("\n" + "=" * 40)
        print("           Fluid → Image Controls")
        print("=" * 40)
        print("  Mouse Drag : Stir the fluid")
        print("  [SPACE]    : Pause / Resume simulation")
        print("  [R]        : Reset the simulation")
        print("  [N]        : Next target image")
        print("  [S]        : Save current frame to 'output.png'")
        print("  [1]        : View Dye")
        print("  [2]        : View Velocity")
        print("  [3]        : View Pressure")
        print("  [4]        : View Divergence")
        print("  [5]        : View Target Image")
        print("=" * 40 + "\n")
    ...
    def switch_to_next_target(self):
        ...
        if len(self.target_images_np) > 1:
            ...
            print(
                f"Switching to target image {self.current_target_index + 1}/{len(self.target_images_np)}"
            )
    ...
    def clear_fields(self):
        ...
        if self.seed_np is not None:
            ...
            avg_colors[:, 0] = ndimage.mean(
                target_float[:, :, 0], labels=labels, index=range(num_cells)
            )
            avg_colors[:, 1] = ndimage.mean(
                target_float[:, :, 1], labels=labels, index=range(num_cells)
            )
            avg_colors[:, 2] = ndimage.mean(
                target_float[:, :, 2], labels=labels, index=range(num_cells)
            )
        ...
        print(
            f"Dye initialized: range [{dye.min():.3f}, {dye.max():.3f}], mean {dye.mean():.3f}"
        )
    ...
    def on_key_event(self, key, action, modifiers):
        ...
        if action == keys.ACTION_PRESS:
            if key == keys.SPACE:
                ...
                self.view_mode = 1
                print("View mode: 1 (Dye)")
            elif key == keys.NUMBER_2:
                self.view_mode = 2
                print("View mode: 2 (Velocity)")
            elif key == keys.NUMBER_3:
                self.view_mode = 3
                print("View mode: 3 (Pressure)")
            elif key == keys.NUMBER_4:
                self.view_mode = 4
                print("View mode: 4 (Divergence)")
            elif key == keys.NUMBER_5:
                self.view_mode = 5
                print("View mode: 5 (Target Image)")
            elif key == keys.S:
                self.save_frame()
            elif key == keys.N:
    ...
    def on_render(self, time: float, frame_time: float):
        if not self.simulation_started:
            self.startup_timer += frame_time
            if self.startup_timer >= self.initial_delay_seconds:
                self.simulation_started = True
                print("✓ Simulation started.")

        # Run simulation steps only if not paused AND the initial delay has passed
        if not self.paused and self.simulation_started:
            ...
            self.prog_advect["dissipation"].value = self.params.vel_dissipation * (
                1.0 - self.params.viscosity
            )
            self.prog_advect["is_dye"].value = 0
            self.quad.render(self.prog_advect)
            self.tex_velocity, self.tex_velocity_prev = (
                self.tex_velocity_prev,
                self.tex_velocity,
            )
            self.fbo_velocity, self.fbo_velocity_prev = (
                self.fbo_velocity_prev,
                self.fbo_velocity,
            )
            ...
            for _ in range(self.params.pressure_iterations):
                ...
                self.tex_pressure, self.tex_pressure_prev = (
                    self.tex_pressure_prev,
                    self.tex_pressure,
                )
                self.fbo_pressure, self.fbo_pressure_prev = (
                    self.fbo_pressure_prev,
                    self.fbo_pressure,
                )
            ...
            current_force = self.params.convergence_force * (
                1.5 - 0.5 * convergence_progress
            )
            ...
            self.prog_apply_forces["settling_strength"].value = (
                self.params.settling_strength
            )
            self.prog_apply_forces["turbulence_strength"].value = (
    ...
    def save_frame(self):
        ...
        img.save(out)
        print(f"✓ Saved frame to {out.resolve()}")
